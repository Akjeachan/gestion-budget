...
namespace GESTIONBUDGETAPI.Controllers
{
    ...
    public class ClotureController : ControllerBase
    {
        ...
        {
            var etat = _budgetContext.Etats.FirstOrDefault(e => e.etat_name == "cloture");
        ...
        {
            var etat = _budgetContext.Etats.FirstOrDefault(e => e.etat_name == "valider");
            if (etat == null)
            {
            ...
            var plannifications = _budgetContext
                .v_Plannifications.Where(p =>
                    p.plan_createdby == id && p.real_actionid == etat.etat_id
                )
                .ToList();
